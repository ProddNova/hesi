...
test('the shipped highway look keeps a nonzero static visibility floor', () => {
  const project = JSON.parse(readFileSync(
    new URL('../../../../data/editor/hesi-world-project.json', import.meta.url),
    'utf8',
  ));
  const lighting = project.environment.lighting;
  assert.ok(lighting.intensity > 0);
  assert.ok(lighting.ambientIntensity > 0);
  assert.ok(lighting.directIntensity > 0);
  assert.notEqual(lighting.tint.toLowerCase(), '#000000');
});
