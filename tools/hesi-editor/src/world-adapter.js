...
function classifyGarageChild(garage, object) {
  // The old procedural-car group is retained only to preserve stable garage
  // child indices for existing projects. It is empty and intentionally hidden.
  if (object === garage.parkedGroup) return { layer: 'Props', type: 'garage-car-legacy-anchor', label: 'Legacy car anchor', editable: false, selectable: false, note: 'empty compatibility anchor; the PSX car uses the next child' };
  // The editor loads the same PSX model the game attaches to carDisplay. Editing
  // this group therefore moves the visible car and emits the exact runtime
  // child-index transform consumed by Apply to Game.
  if (object === garage.carDisplay) return { layer: 'Props', type: 'garage-parked-car', label: 'Japan Sedan', editable: true, note: 'shared PSXStyle showroom anchor used by the game' };
...
async function makeGarageWorld(onProgress) {
  ...
  const [{ GarageSystem }, Data, carPack] = await Promise.all([
    import('/js/garage.js'),
    import('/js/data.js'),
  ...
  garage.refreshCar(starterSpec);
  onProgress(`Loading ${carPack.getPSXCarModel(carPack.DEFAULT_PSX_CAR_ID).label} showroom car`);
  const showroomCar = await carPack.loadPSXCar(carPack.DEFAULT_PSX_CAR_ID, {
    length: starterSpec.dimensions?.length || starterSpec.length,
    color: starterSpec.color,
  });
  garage.carDisplay.add(showroomCar);
  garage.carDisplay.visible = true;
  garage.refreshColliders();
