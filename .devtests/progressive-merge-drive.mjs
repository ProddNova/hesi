/** Dynamic VehiclePhysics traversal gate for the four progressive prototypes. */
...
for (const transition of map.progressiveTransitions) {
  ...
  const end = transition.branchInterval[1] - 2;
  const first = map.sampleLane(route.id, start, 0, 1);
  ...
  for (let step = 0; step < 60 * 30; step += 1) {
    ...
    const lane = map.sampleLane(route.id, projection.distance, 0, 1);
    ...
    const target = map.sampleLane(route.id, Math.min(end, projection.distance + 14), 0, 1);
  ...
  if (!reached) failures.push(`${transition.id}: stopped at ${lastDistance.toFixed(1)} / ${end.toFixed(1)} m`);
  if (!finite) failures.push(`${transition.id}: non-finite vehicle state`);
  if (collisionEvents) failures.push(`${transition.id}: ${collisionEvents} collision event(s)`);
  if (maximumWallCorrection > 0.001) {
    failures.push(`${transition.id}: wall correction ${maximumWallCorrection.toFixed(3)} m`);
  }
  if (maximumLaneError > 1) failures.push(`${transition.id}: lane error ${maximumLaneError.toFixed(2)} m`);
  if (ownershipSequence.length > 2) {
    failures.push(`${transition.id}: ownership oscillation ${ownershipSequence.join(' -> ')}`);
  }

  console.log(
    `${transition.id} ${reached ? 'PASS-CANDIDATE' : 'FAIL-CANDIDATE'} `
