...
class ShutokoNights {
  ...
  installP4CorridorLegend(){
    ...
    legend.innerHTML=`<div style="color:#fff;font-size:15px">P4 ${transition.id}</div><div><span style="color:#29e6ff">━━</span> auxiliary centre</div><div><span style="color:#ffa52f">━━</span> inner boundary</div><div><span style="color:#ff3fd2">━━</span> outer boundary</div><div><span style="color:#70ff55">━━</span> target r1_0:${transition.branchFeedLane} geometry</div><div><span style="color:#fff">┃</span> physical width · min ${minimum.toFixed(2)} m</div><div><span style="color:#ffe52a">┃</span> marked width · ${transition.auxiliaryMarkedWidth.toFixed(2)} m</div><div><span style="color:#ffee22">●</span> ownership @ host ${transition.transferComplete.toFixed(2)} / branch ${transition.transferCompleteBranch.toFixed(2)}</div>`;
